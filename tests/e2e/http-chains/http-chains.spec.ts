...
test.describe('Spritecloud — HTTP redirect chains for https://www.spritecloud.com', () => {
  test('@kind:http-chain @redirect-chain follows multi-hop redirects', async ({ request }) => {
    // 301 + 302 + 200 — typical CDN → canonical → asset redirect chain.
  ...
  test('@kind:http-chain @410 nonexistent paths return a user-friendly error', async ({ request }) => {
  ...
  test('@kind:http-chain @429 server-side rate-limit response behavior', async ({ context, page }) => {
    let firstCall = true
