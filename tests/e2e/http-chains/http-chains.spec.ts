...
test.describe('ING — HTTP redirect and error chains for https://www.ing.nl', () => {
  test('@kind:http-chain follows multi-hop HTTP redirects (301 → 302 → 200)', async ({ request }) => {
  ...
  test('@kind:http-chain returns a user-friendly error for nonexistent (410 Gone) paths', async ({ request }) => {
  ...
  test('@kind:http-chain handles server-side rate-limiting (429 with Retry-After) without crashing', async ({ context, page }) => {
    let firstCall = true
