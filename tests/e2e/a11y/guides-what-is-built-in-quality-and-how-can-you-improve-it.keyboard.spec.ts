 ...
 *
 *   smoke           — Tab key navigates through the first 10 focusable elements
 *   focus-indicator — every focusable element shows a visible focus indicator
 *   escape-dismiss  — Escape key closes an open modal or dialog and restores focused element
 *   enter-space     — Enter activates links and buttons; Space activates buttons
 *   no-trap         — Tabbing past the last focusable wraps around or exits (no focus trap)
 *
...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/guides/what-is-built-in-quality-and-how-can-you-improve-it', () => {
  ...
  test('@kind:keyboard @no-trap Tabbing past the final focusable wraps or exits the page', async ({ page }) => {
