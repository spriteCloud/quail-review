 ...
 *   no-aria-hidden      — No interactive elements are inside aria-hidden containers subtrees
 *   heading-hierarchy   — Headings follow a logical sequential order (no skipped levels) (no h1→h3 jumps)
...
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/devops', () => {
  test('@kind:a11y-landmarks @smoke Page contains exactly one main region, one h1 heading, and at least one nav region', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @negative No interactive elements are inside aria-hidden containers', async ({ page }) => {
    await page.goto('/devops', { waitUntil: 'domcontentloaded' })
  ...
  test('@kind:a11y-landmarks @heading-hierarchy Headings follow a logical sequential order (no skipped levels)', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @landmark-names Duplicate landmarks have accessible names for screen reader distinction', async ({ page }) => {
