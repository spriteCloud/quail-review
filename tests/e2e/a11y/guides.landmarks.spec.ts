 ...
 *
 *   smoke               — single main region, single h1, at least one nav
 *   no-aria-hidden      — no interactive elements inside aria-hidden sections
 *   heading-hierarchy   — headings increase by one level at most (no skipped levels)
...
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/guides', () => {
  test('@kind:a11y-landmarks smoke: single main, h1, and nav regions', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks negative: no focusable elements in aria-hidden', async ({ page }) => {
    await page.goto('/guides', { waitUntil: 'domcontentloaded' })
  ...
  test('@kind:a11y-landmarks heading hierarchy: sequential heading levels', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks landmark names: duplicate landmarks have names', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks skip link: "skip to content" affordance is present', async ({ page }) => {
