 ...
 *   escape-dismiss — Escape closes a dialog/modal and restores focus to the trigger
 *   enter-space — Enter activates links and buttons; Space activates buttons only
 *   no-trap — Tabbing past the last focusable wraps or exits (no trapping)
 *
...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/devops', () => {
  test('@kind:keyboard @smoke: Tab cycles through the first 10 keyboard-focusable elements', async ({ page }) => {
  ...
  test('@kind:keyboard @focus-indicator: First keyboard-focused element has a visible focus style', async ({ page }) => {
    await page.goto('/devops', { waitUntil: 'domcontentloaded' })
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @no-trap: Tabbing past the last focusable wraps around or exits the page', async ({ page }) => {
