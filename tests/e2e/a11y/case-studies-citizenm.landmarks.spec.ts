 ...
 *
 *   smoke               — single main element, single h1 heading, at least one navigation region
 *   no-aria-hidden      — no interactive elements inside aria-hidden regions subtrees
 *   heading-hierarchy   — headings follow proper hierarchical order (no skipped levels) (no h1→h3 jumps)
 *   landmark-names      — repeated landmarks have unique accessible names
 *   skip-link           — "skip to content" link or in-page main target present (WCAG 2.4.1)
 *
...
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/case-studies/citizenm', () => {
  test('@kind:a11y-landmarks smoke test: single main, h1, and nav', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @heading-hierarchy headings follow proper hierarchical order (no skipped levels)', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks landmark names test: repeated landmarks named', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks skip-link test: skip-to-content affordance present', async ({ page }) => {
