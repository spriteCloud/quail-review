...
test.describe('Spritecloud — accessible landmark checks on the shift-left testing guide', () => {
  test('@kind:a11y-landmarks ensures one main landmark, one level-1 heading, and at least one navigation landmark', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks confirms no interactive elements exist inside aria-hidden regions', async ({ page }) => {
    await page.goto('/guides/guide-implementing-shift-left-testing-in-an-agile-environment', { waitUntil: 'domcontentloaded' })
  ...
  test('@kind:a11y-landmarks validates headings descend logically without skipping levels', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks ensures multiple landmarks of the same type each have a distinguishing label', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks verifies presence of a skip-to-content link for keyboard navigation (WCAG 2.4.1)', async ({ page }) => {
