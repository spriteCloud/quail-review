...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/guides/guide-implementing-shift-left-testing-in-an-agile-environment', () => {
  ...
  test('@kind:keyboard @escape-dismiss Pressing Escape closes a visible dialog and returns focus to the trigger', async ({ page }) => {
    ...
    expect.soft(stillOpen, 'dialog should close when pressing Escape').toBe(false)
  })
  ...
  test('@kind:keyboard @no-trap Tab from the last focusable wraps or exits the page', async ({ page }) => {
    ...
    expect.soft(trapped, `Tab past the last focusable should wrap or exit; observed transition from ${firstActiveTag} to ${secondActiveTag}`).toBe(false)
  })
})
