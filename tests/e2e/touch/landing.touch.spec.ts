...
test.describe('Spritecloud: Touch Gestures @ https://www.spritecloud.com/', () => {
  test('Long-press on primary CTA does not double-fire (@smoke)', async ({ page }) => {
  ...
  test('Rapid vertical swipes do not break layout', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' })
    const viewport = page.viewportSize() || { width: 390, height: 844 }
    const cx = viewport.width / 2
  ...
  test('Tap during orientation change does not duplicate-fire', async ({ browser }) => {
