 ...
 *
 * Run with: `npx playwright test --grep @kind:touch`
 */
...
test.describe('Spritecloud — touch gestures @ https://www.spritecloud.com/contact', () => {
  test('@kind:touch @smoke long-press on primary CTA does not fire click more than once', async ({ page }) => {
    ...
    // A single single tap should trigger exactly one click event.
    expect.soft(clicks, 'single tap should trigger exactly one click event').toBeLessThanOrEqual(1)
  })
  ...
  test('@kind:touch @pinch-zoom synthesized pinch-zoom gesture preserves page interactivity', async ({ page }) => {
    await page.goto('/contact', { waitUntil: 'domcontentloaded' })
    const viewport = page.viewportSize() || { width: 390, height: 844 }
  ...
  test('@kind:touch @scroll-momentum rapid vertical swipes do not corrupt the page layout', async ({ page }) => {
    await page.goto('/contact', { waitUntil: 'domcontentloaded' })
    const viewport = page.viewportSize() || { width: 390, height: 844 }
    const cx = viewport.width / 2
  ...
  test('@kind:touch @tap-then-rotate tap during orientation change does not fire duplicate click events', async ({ browser }) => {
