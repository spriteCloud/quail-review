...
func runHealLocal(ctx context.Context, cfg config.Config) error {
	...
	auto, needsReview, blocked := heal.SplitByStatus(edits)
	rlog.Info("heal: applied edits locally", "auto", len(auto), "needs-review", len(needsReview), "blocked", len(blocked), "files", len(patched))
	return nil
}
...
func emitOrSkipHealOutput(edits []heal.Edit, cfg config.Config, client *gh.Client, prInfo *prSummary) bool {
	auto, needsReview, blocked := heal.SplitByStatus(edits)
	if len(auto) == 0 {
		rlog.Info("no auto-appliable locator edits", "needs-review", len(needsReview), "blocked", len(blocked))
		if len(needsReview) > 0 || len(blocked) > 0 {
			writeStepSummary(fmt.Sprintf("quail: no locator edits applied automatically — %d need review, %d blocked. See details below.\n", len(needsReview), len(blocked)))
			printEdits(edits)
		} else {
...
func openHealPR(ctx context.Context, client *gh.Client, cfg config.Config, files []diff.File, prInfo *prSummary, edits []heal.Edit) error {
	...
	url, err := client.OpenPR(ctx, gh.PROpts{
		...
		Body:  healPRBody(prInfo, auto, needsReview, blocked),
		Files: patched,
	})
	if err != nil {
		return err
	}
	rlog.Info("opened heal PR", "url", url, "auto", len(auto), "needs-review", len(needsReview), "blocked", len(blocked))
	writeStepSummary(healSummary(prInfo, auto, needsReview, blocked, url))
	return nil
}
...
// applyEdits materializes only StatusAuto edits — StatusNeedsReview and
// StatusBlocked are surfaced (see printEdits/emitOrSkipHealOutput) but
// never written to disk without a human picking between candidates or
// resolving the block.
func applyEdits(workDir string, indexed map[string]string, edits []heal.Edit) map[string][]byte {
	out := map[string][]byte{}
	bucket := map[string][]heal.Edit{}
	for _, e := range edits {
		if e.Status != heal.StatusAuto {
			continue
		}
...
func healPRBody(pr *prSummary, auto, needsReview, blocked []heal.Edit) string {
	var b strings.Builder
	if pr != nil {
		fmt.Fprintf(&b, "Generated by quail for #%d.\n\n", pr.Number)
	}
	b.WriteString("Locator edits applied:\n")
	for _, e := range auto {
		fmt.Fprintf(&b, "- `%s:%d` — %s\n", e.File, e.Line, e.Reason)
	}
	if len(needsReview) > 0 {
		b.WriteString("\nNeeds review (not applied — pick a candidate):\n")
...
func healSummary(pr *prSummary, auto, needsReview, blocked []heal.Edit, url string) string {
	var b strings.Builder
	...
	b.WriteString("| File | Line | Status | Reason |\n")
	b.WriteString("|---|---|---|---|\n")
	for _, e := range auto {
		fmt.Fprintf(&b, "| `%s` | %d | applied | %s |\n", e.File, e.Line, e.Reason)
	}
	for _, e := range needsReview {
		fmt.Fprintf(&b, "| `%s` | %d | needs review | %s |\n", e.File, e.Line, e.Reason)
	}
	for _, e := range blocked {
		fmt.Fprintf(&b, "| `%s` | %d | blocked | %s |\n", e.File, e.Line, e.Reason)
	}
	b.WriteString("\n")
	return b.String()
}
